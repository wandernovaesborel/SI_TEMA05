...
def analise_sentimento(texto):
    ...
    expressoes = get_expressoes()

    pontuacao = 0
    for token in texto.split():
        if token in palavras_positivas:
            peso = get_peso_palavra(token, 'positiva')
            pontuacao += peso
        elif token in palavras_negativas:
            peso = get_peso_palavra(token, 'negativa')
            pontuacao -= peso

    # Verifica se há presença de expressões positivas na frase
    for expressao, peso in expressoes['positiva'].items():
        palavras_expressao = expressao.split()
        if all(palavra in texto for palavra in palavras_expressao):
            pontuacao += peso

    # Verifica se há presença de expressões negativas na frase
    for expressao, peso in expressoes['negativa'].items():
        palavras_expressao = expressao.split()
        if all(palavra in texto for palavra in palavras_expressao):
            pontuacao -= peso
...
# Função para obter expressões do banco de dados com seus tipos
def get_expressoes():
    conn = mysql.connector.connect(**db_config)
    cursor = conn.cursor()
    cursor.execute("SELECT expressao, peso, tipo FROM expressoes")
    expressoes = {'positiva': {}, 'negativa': {}}
    for expressao, peso, tipo in cursor.fetchall():
        expressoes[tipo][expressao] = peso
    conn.close()
    return expressoes
...
# Rota para adicionar palavras positivas, negativas e expressões
...
def adicionar_palavra():
    ...
    elif tipo in ['expressao_positiva', 'expressao_negativa']:
        tipo = 'positiva' if tipo == 'expressao_positiva' else 'negativa'
        cursor.execute("INSERT INTO expressoes (expressao, peso, tipo) VALUES (%s, %s, %s)", (palavra, peso, tipo))
